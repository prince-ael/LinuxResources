...
//      Completed (8) Display the summary that you created above
        ...
        long date = mCursor.getLong(MainActivity.POSITION_DATE);
        String dateStr = SunshineDateUtils.getFriendlyDateString(mContext , date , true);

        double maxTemp = mCursor.getDouble(MainActivity.POSITION_MAX_TEMP);
        double minTemp = mCursor.getDouble(MainActivity.POSITION_MIN_TEMP);

        int weatherId = mCursor.getInt(MainActivity.POSITION_WEATHER_ID);
        String descripton = SunshineWeatherUtils.getStringForWeatherCondition(mContext , weatherId);

        String maxAndMin = SunshineWeatherUtils.formatHighLows(mContext ,  maxTemp , minTemp);

        forecastAdapterViewHolder.weatherSummary.setText(dateStr+" - "+descripton+" - "+maxAndMin);
