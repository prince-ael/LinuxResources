...
//  Completed (1) Create a constant int value to identify the notification
    private static final int WEATHER_NOTIFICATION_ID = 3647;
    private static final int PENDING_INTENT_REQUEST_CODE = 214;
...
//          Completed (4) Use TaskStackBuilder to create the proper PendingIntent
            TaskStackBuilder activityStack = TaskStackBuilder.create(context);
            activityStack.addParentStack(MainActivity.class);
            activityStack.addNextIntent(detailActivityIntent);

//          Completed (5) Set the content Intent of the NotificationBuilder
            PendingIntent pendingIntent = activityStack.getPendingIntent(PENDING_INTENT_REQUEST_CODE
                    ,PendingIntent.FLAG_UPDATE_CURRENT);

            notificationBuilder.setLargeIcon(largeIcon);
            notificationBuilder.setSmallIcon(smallArtResourceId);
...
//          Completed (6) Get a reference to the NotificationManager
            NotificationManager notificationMnager = (NotificationManager) context.getSystemService(Context.NOTIFICATION_SERVICE);

//          Completed (7) Notify the user with the ID WEATHER_NOTIFICATION_ID
            notificationMnager.notify(WEATHER_NOTIFICATION_ID , notificationBuilder.build());
